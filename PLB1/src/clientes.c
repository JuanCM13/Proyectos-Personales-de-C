...
int desplegarMenu(int* respuesta)
{
    ...
    if(respuesta != NULL)
    {
        ...
        printf("|   11) Cliente con más pedidos pendientes  NO ANDA!! :(    Esta arrancado..                   |\n");
        printf("|   0)  SALIR                                                                                  |\n");
        printf("|----------------------------------------------------------------------------------------------|\n\n");

        if(Get_numeroConLimites("SELECCIONE UNA OPCION..", "ERROR, INGRESE UNA OPCION ENTRE LAS MOSTRADAS..",0,13,&aux) == 0)
        {
            ret = 0;
            *respuesta = aux;
        }
...
eCliente generarCliente(int* id)
{
    ...
    if(Get_NombreEmpresa("Ingrese el nombre de la empresa.." , "El nombre de la empresa no puede ser tan grande.." , auxiliar.nombreCliente , sizeof(auxiliar.nombreCliente)) != 0)
    {
        printf("No se pudo cargar el nombre de la empresa, reintente..\n");
    }
    else
    {
        OrdenarNombre(auxiliar.nombreCliente , sizeof(auxiliar.nombreCliente));
    }

    if(Get_cuit("Ingrese el cuit, 11 numeros sin guiones ni espacios..", "Respete el formato del ejemplo: EJ: 20123456789", auxiliar.cuitCliente , sizeof(auxiliar.cuitCliente)) != 0)
    {
        printf("Algo fallo con la carga.. por favor reintente..\n");
    }


    if(Get_cadena("Ingrese la direccion donde vive...", "Error, la direccion no puede ser tan larga, reingrese..",auxiliar.direccionCliente, sizeof(auxiliar.direccionCliente)) != 0)
    {
        printf("Algo fallo con la carga.. por favor reintente..\n");
    }
    else
    {
        OrdenarNombre(auxiliar.direccionCliente , sizeof(auxiliar.direccionCliente));
    }
