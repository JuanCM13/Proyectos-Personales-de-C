...
LinkedList* ll_map(LinkedList* this , int(*fn)(void* pElement) )
{
	...
	if(this != NULL && fn != NULL)
	{
		...
		if(largo > 0)
		{
			listaConModificaciones = ll_clone(this); //esto esta al pedo, onda los punteros a elemetos son los mismos asique no se modifican
...
int ll_sort2(LinkedList* this, int (*pFunc)(void* ,void* , LinkedList*), int order , LinkedList* this2) //para este ejercicio duh
{
	int returnAux = -1;
	int i;
	int j;
	...
	if(this != NULL && pFunc != NULL && (order == 1 || order == 0) && this2 != NULL )
	{
		largo = ll_len(this);
		...
		for(i=0 ; i<largo-1 ; i++)
		{
			...
			for(j=i+1 ; j<largo ; j++)
			{
				...
				if( (order == 1 && pFunc(auxUnoPelement , auxDosPelement , this2) > 0) || (order == 0 && pFunc(auxUnoPelement , auxDosPelement , this2) < 0) )
				{
					auxSwap = auxUnoPelement;
					ll_set(this , i , auxDosPelement);
					ll_set(this , j , auxSwap);
